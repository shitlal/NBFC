...
public class JasperReportASFController {
	...
	public ModelAndView taxInvoiceReportASF(
		...
		if (mliId != null) {
			list = prepareTaxInvoiceDataListofBean(nbfcInvoiceService.getTaxInvoiceDataASF(mliId, session));
	...
	public ModelAndView callJasperReportOfProformaASF(
			@ModelAttribute("command") CGPANDetailsReportBean bean,
			ModelAndView modelAndView, @RequestParam("danId") String danId,@RequestParam("year") String year,
		...
		if (proformainvoiceCount == 0) {
			List<Object> list = nbfcInvoiceService.getProformaReportDetailsASF(danId,userId,year);
			...
			while (itr1.hasNext()) {
				...
				Double DAN_AMT = ((BigDecimal) obj1[11]).doubleValue();
				Double BASE_AMT = ((BigDecimal) obj1[12]).doubleValue();
				...
				bean.setDan_Amt(DAN_AMT);
				bean.setDci_base_amt(BASE_AMT);
			}
			...
			while (itr1.hasNext()) {
				...
				bean.setTaxid((ProformaInvoiceId));
				bean.setDanId(danId);
				bean.setAsfYear(year);
				bean.setOutstandingAmount1(oustandingAmt);
				bean.setDciGuaranteeeSDate(danGenDate1);
			...
			nbfcInvoiceService.callProformaInvoiceReportASF(danId, response, request, bean);
	...
	private ProformaInvoiceDetailsASF prepareModelForProformaDetails(
		...
		proformaDetailsObj1.setDAN_TYPE("AF");
	...
	public ModelAndView callJasperReportOfTaxASF(
		...
		// CGPANDetailsReportBean bean=null;
		String userId = (String) session.getAttribute("userId");
		Integer TaxinvoiceCount = nbfcInvoiceService.getTaxInvoiceIdCountASF(danId);
		//if (TaxinvoiceCount == 0) {
			//List<Object> list = nbfcInvoiceService.getTaxReportDetailsASF(danId,userId,year);
			// need to change here for new query 
			List<Object> list = nbfcInvoiceService.getTaxReportDetailsASF(danId,userId,year);
			Iterator<Object> itr1 = list.iterator();
			while (itr1.hasNext()) {
				Object[] obj1 = (Object[]) itr1.next();
...
//				String danGenDate1 = df.format(danGenDate);
			 ...
			 */
				...
				String taxInvoiceId = nbfcInvoiceService.getTaxInvoiceNo(danId);
			/*
			 * if (maxTaxInvoiceIdCount == 0) { GeneratedTaxNo = (long) 1000; } else {
			...
			nbfcInvoiceService.callTaxInvoiceReportASF(danId, response, request,bean);
			/*
			 * TaxInvoiceDetailsASF taxDetailsObj = prepareModelForDetails(bean, danId);
	...
	/*
			 ...
			 * nbfcInvoiceService.getTaxReportDetailsASF(danId,userId); Iterator<Object>
			 * itr1 = list.iterator(); while (itr1.hasNext()) { Object[] obj1 = (Object[])
			 * itr1.next(); String danID = (String) obj1[0]; Double igstAmt = ((BigDecimal)
			 * obj1[1]).doubleValue(); Double cgstAmt = ((BigDecimal)
			 * obj1[2]).doubleValue(); Double sgstAmt = ((BigDecimal)
			 * obj1[3]).doubleValue(); Double oustandingAmt = ((BigDecimal)
			 * obj1[4]).doubleValue(); Date appropriationDate = (Date) obj1[5];
			 * System.out.println("----date--------" + appropriationDate); SimpleDateFormat
			 * df = new SimpleDateFormat("dd/MM/yyyy"); String appropriationDate1=""; String
			 * appDate=""; if(appropriationDate !=null){
			 * 
			 * appropriationDate1 = df.format(appropriationDate); } Float guranteeFee =
			 * ((BigDecimal) obj1[6]).floatValue(); Date danGenDate = (Date) obj1[7]; String
			 * danGenDate1 = df.format(danGenDate); String stateCode = (String) obj1[8];
			 * String stateName = (String) obj1[9]; Date date1 = (Date) obj1[10]; Double
			 * DAN_AMT = ((BigDecimal) obj1[11]).doubleValue(); Double BASE_AMT =
			 * ((BigDecimal) obj1[12]).doubleValue(); appDate = df.format(date1);
			 * bean.setAppsubmittedDate(appDate); String TaxInvoiceId =
			 * nbfcInvoiceService.getTaxInvoiceNoASF(danId);
			 * 
			 * System.out.println("TaxInvoiceId-------------" + TaxInvoiceId);
			 * bean.setTaxInvoiceId(Long.parseLong(TaxInvoiceId)); bean.setDanId(danId);
			 * bean.setOutstandingAmount1(oustandingAmt);
			 * bean.setDciAppropriationDate(appropriationDate1); bean.setIgstAmt(igstAmt);
			 * bean.setCgstAmt(cgstAmt); bean.setSgstAmt(sgstAmt);
			 * bean.setGuaranteeFee(guranteeFee); bean.setState(stateName);
			 * bean.setDan_Amt(DAN_AMT); bean.setDci_base_amt(BASE_AMT); }
			 * nbfcInvoiceService.callTaxInvoiceReportASF(danId, response, request, bean);
			 * // TaxInvoiceDetails taxDetailsObj = // prepareModelForDetails(bean,dan_id);
			 * // nbfcInvoiceService.insertTaxInvoiceDetails(taxDetailsObj);
			 * 
			 * }
			 */
	...
	private TaxInvoiceDetailsASF prepareModelForDetails(CGPANDetailsReportBean bean, String PORTFOLIONAME) {
	...
	/*START Add By VinodSingh on 01-JAN-2021 For ASF Tax invoice Report generation */ 
	@RequestMapping(value = "/genPDfSuccessInvoiceIRNForASF", method = RequestMethod.GET)
	public ModelAndView generatePDfSuccessInvoiceIRNForASF(@ModelAttribute("command") CGPANDetailsReportBean bean,ModelAndView modelAndView, @RequestParam("danId") String danId,@RequestParam("year") String year, @RequestParam("bankname") String bankname,
			Model modelMsg, ModelMap model, HttpServletResponse response,HttpServletRequest request,HttpSession session) throws Exception {

		System.out.println("Test" + danId+" ,bankname::"+bankname+" , Year::"+year);
		String userId = (String) session.getAttribute("userId");
		...
		try
		{
			System.out.println(request.getParameter("dTaxInvId")+"<<<<<<<< FINDED <<<<<<<<<<<<<<<<<bankname::"+bankname);
			List<Object> list = nbfcInvoiceService.getTaxReportDetailsASF(danId,userId,year);
			Iterator<Object> itr1 = list.iterator();
			while (itr1.hasNext()) {
				Object[] obj1 = (Object[]) itr1.next();
				String danID = (String) obj1[0];				
				BigDecimal igstAmt = ((BigDecimal) obj1[1]).abs();
				BigDecimal cgstAmt = ((BigDecimal) obj1[2]).abs();
				BigDecimal sgstAmt = ((BigDecimal) obj1[3]).abs();
				BigDecimal ouStandingAmt = ((BigDecimal) obj1[4]).abs();
				...
				BigDecimal guranteeFee = ((BigDecimal) obj1[6]).abs();				
				Date danGenDate = (Date) obj1[7];//GUARANTEE START DATE

				String guaranteeeStartDate = df.format(danGenDate);
				String stateCode = (String) obj1[8];
				String stateName = (String) obj1[9];
				Date date1 = (Date) obj1[10];
				
				BigDecimal DAN_AMT = ((BigDecimal) obj1[11]).abs();
				BigDecimal BASE_AMT = ((BigDecimal) obj1[12]).abs();
				
				String gstNo = (String) obj1[13];
				String address = (String) obj1[14];
				appDate = df.format(date1);
				bean.setAppsubmittedDate(appDate);
				bean.setAsfYear(year);

				String taxInvoiceId = nbfcInvoiceService.getTaxInvoiceNo(danId);
				System.out.println(":::::::::::::TaxInvoiceId:::TAX_INV_ID::::::::::::::" + taxInvoiceId);	

				String signNo="";
				String irnNo="";				
				List<Object> QRlist1= nbfcInvoiceService.getTaxQRCode(taxInvoiceId);
				if(QRlist1!=null) {
					...
					while (QRitr.hasNext()) {
						...
						System.out.println("IRN No::::::"+irnNo+" ::,signNo:::::::::"+signNo);
					}
					bean.setIrnNO(irnNo!=null ? irnNo : "");
					bean.setqRcode(signNo!=null ? signNo : "");
					System.out.println(":::::::Get The IRN No:::::::::::"+bean.getIrnNO());
				}

				bean.setDciGuaranteeeSDate(guaranteeeStartDate);
				bean.setPortfolioName(danId);
				bean.setBankname(bankname);
				bean.setTaxid(taxInvoiceId);
				bean.setDanId(danID);							
				bean.setDciAppropriationDate(appropriationDate1);
				bean.setIgstAmount(igstAmt);
				bean.setCgstAmount(cgstAmt);
				bean.setSgstAmount(sgstAmt);
				bean.setGuaranteeAmt1(guranteeFee);
				bean.setState(stateName);
				bean.setDanAmount(DAN_AMT);//Total Amount
				bean.setBaseAmount(BASE_AMT);
				bean.setGstNo(gstNo);
				bean.setStateCode(stateCode);
				bean.setAddress(address);
				bean.setOutStandingAmount(ouStandingAmt);				
			}

			Calendar cal = Calendar.getInstance();
			SimpleDateFormat sdf = new SimpleDateFormat("ddMMyyyyHHmmss");
			String strDate = "Invoice".concat(sdf.format(cal.getTime()));
			String pdfStatus = nbfcInvoiceService.genPdfTaxInvoiceForASF(response,bean,contextPath, strDate);
			System.out.println("::::::::::pdf Generation Status:::::::::::::"+pdfStatus);
